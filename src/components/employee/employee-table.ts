...
export class EmployeeTable extends LitElement {
    ...
    static styles = css`
    :host { display:block; }
    .table-container{ margin:0 24px; padding:16px; background:#fff; border:1px solid #eee; border-radius:12px; overflow-x:auto; }
    table{ width:100%; border-collapse:collapse; font-size:14px; }
    thead th{
      font-weight:700; color:var(--color-primary,#ff6a00); text-align:left; font-size:13px;
      padding:14px 12px; border-bottom:1px solid #f0f0f0; white-space:nowrap; background:#fff;
    }
    tbody td{ padding:16px 12px; border-bottom:1px solid #f4f4f4; white-space:nowrap; vertical-align:middle; color:var(--color-text,#2b2b2b); }
    ...
    /* turuncu checkbox */
    input[type="checkbox"]{ width:16px; height:16px; accent-color: var(--color-primary, #ff6a00); }

    .first{ font-weight:600; }

    .actions-cell{ display:flex; gap:12px; justify-content:flex-end; }
    .icon-btn{
      border:0; background:transparent; cursor:pointer; padding:0; line-height:0;
